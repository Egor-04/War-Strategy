...
public class UnitMovement : MonoBehaviour
{
    ...
    [Tooltip("»звлеченна€ информаци€ из AttackBehaviour")]
        ...
        if (MovementTarget)
        {
            if (MovementTarget.GetComponent<Unit>() || MovementTarget.GetComponent<Building>())
            {
                ...
                if (currentTargetDistance <= _minAttackDistance)
                {
                    _navMeshAgent.enabled = false;
                    transform.LookAt(MovementTarget);
                    _attackBehavour.SetNearbyBattleTarget(MovementTarget);// «десь из за услови стрел€ет один раз а не посто€нно, потому что тут цель занул€етс€
                    transform.eulerAngles = new Vector3(0f, transform.eulerAngles.y, 0f);
                    MovementTarget = null;
                }
            }
